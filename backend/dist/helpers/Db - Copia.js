...
const getBotTicket = async (contato) => {
	const connection = await createConnection();
	const [rows] = await connection.execute("SELECT t.chatbot FROM Tickets where id = " + contato );
	if (rows.length > 0) return rows[0].chatbot;
	return false;
}

const setBotTicket = async (ipid) => {
	const connection = await createConnection();
	const [rows] = await connection.execute("update Tickets set chetbot = 'S' where id = " + ipid );
	if (rows.length > 0) return rows;
	return false;
}
