...
const GetLimiteWhatsApp = __importDefault(require("../../helpers/GetLimiteWhatsApp"));
const CreateWhatsAppService = ({ name, status = "OPENING", queueIds = [], greetingMessage, farewellMessage, isDefault = false }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let WhatsAppLimit = yield Whatsapp.findAll();
    const limite = yield GetLimiteWhatsApp.default();
    try {
        if (WhatsAppLimit.length > limite - 1)
            {
                throw new AppError("Limite de conexões atingida", 404)
            }
    }
    catch {
        throw new AppError("Limite de conexões atingida", 404)
